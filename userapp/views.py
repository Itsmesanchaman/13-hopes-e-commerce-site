...
def user_category(request):
    if request.user.is_authenticated:
        ...
        if Userprofile.objects.filter(user=user).exists():
            ...
            return render(request, 'userapp/category.html',
                      {'product_data': product, 'category_data': category, 'no': item_count, 'userdetails': user_details})
        else:
            return render(request, 'userapp/category.html',
...
def members(request):
    if request.user.is_authenticated:
        ...
        if Userprofile.objects.filter(user=user).exists():
            user_details = Userprofile.objects.get(user=user)
            return render(request, 'userapp/member.html',
                {'product_data': product, 'category_data': category, 'no': item_count, 'userdetails': user_details})
        else:
            return render(request, 'userapp/member.html',
                        {'product_data': product, 'category_data': category, 'no': item_count})
...
def gallery(request):
    if request.user.is_authenticated:
        ...
        if Userprofile.objects.filter(user=user).exists():
            user_details = Userprofile.objects.get(user=user)
            return render(request, 'userapp/imgGallery.html',
                    {'product_data': product, 'category_data': category, 'no': item_count, 'userdetails': user_details})
        else:
            return render(request, 'userapp/imgGallery.html',
                            {'product_data': product, 'category_data': category, 'no': item_count})
    ...
    if request.user.is_authenticated:
        ...
        if Userprofile.objects.filter(user=user).exists():
            ...
            return render(request, 'userapp/user_address.html',
                      {'cart_data': cart, 'category_data': category, 'no': item_count, 'userdetails': user_details, 'address': address})
        else:
            return render(request, 'userapp/user_address.html',
...
def create_address(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if Userprofile.objects.filter(user=user).exists():
                user_details = Userprofile.objects.get(user=user)
                return redirect(show_address, {'userdetails': user_details})
            else:
